...
export const PaymentManagementPage = () => {
  ...
  const onCreate = async (data: Partial<Payment>) => {
  ...
  const onUpdate = async (key: string, data: Partial<Payment>) => {
    const resp = await api.updatePayment(key, data);
    if (resp.isSuccess) {
  ...
  const handleDeleteRows = async (rows: string[]) => {
    ...
    const resp = await api.deletePayments(rows);
    if (resp.isSuccess) {
      toast.success(t("Delete Successfully"));
      await refetch();
    } else {
  ...
  const handleUploadFile = async (file: File, progressCallback?: Function) => {
    const resp = await api.uploadPaymentFile(file);
  ...
  const handleDownloadTemplate = async () => {
    const resp = await api.downloadTemplatePaymentFile();
    if (resp.isSuccess) {
      ...
      <AdminContentLayout.Slot name={"Header"}>
        <PageHeaderLayout>
          <PageHeaderLayout.Slot name={"Before"}>
            <div className="font-bold dx-font-m">{t("Payment Management")}</div>
