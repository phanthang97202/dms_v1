...
export const PortManagementPage = () => {
  ...
  const onCreate = async (data: Partial<Port>) => {
    const resp = await api.createPort({
      ...data,
    });
    if (resp.isSuccess) {
  ...
  const onUpdate = async (key: string, data: Partial<Port>) => {
    const resp = await api.updatePort(key, data);
    if (resp.isSuccess) {
  ...
  const onDelete = async (key: string) => {
    const resp = await api.deletePort(key);
    if (resp.isSuccess) {
  ...
  const handleDeleteRows = async (rows: any) => {
    const resp = await api.deletePorts(rows);
    if (resp.isSuccess) {
      toast.success(t("Delete Successfully"));
      await refetch();
    } else {
  ...
  const handleSavingRow = async (e: any) => {
    ...
    if (e.changes && e.changes.length > 0) {
      ...
      if (type === "remove") {
        ...
        const data: Port = e.changes[0].data!;
  ...
  const handleUploadFile = async (file: File, progressCallback?: Function) => {
    const resp = await api.uploadPortFile(file);
  ...
  const handleDownloadTemplate = async () => {
    const resp = await api.downloadTemplatePortFile();
    if (resp.isSuccess) {
