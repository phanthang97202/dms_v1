...
export const DirCAManagementPage = () => {
  const { t } = useI18n("DirCA");
  ...
  const onCreate = async (data: Partial<DirCA>) => {
    const resp = await api.createDirCA({
      ...data,
    });
  ...
  const onUpdate = async (key: string, data: Partial<DirCA>) => {
    const resp = await api.updateDirCA(key, data);
    if (resp.isSuccess) {
  ...
  const onDelete = async (key: string) => {
    const resp = await api.deleteDirCA(key);
    if (resp.isSuccess) {
  ...
  const handleDeleteRows = async (rows: any) => {
    const resp = await api.deleteDirCAs(rows);
    if (resp.isSuccess) {
      toast.success(t("Delete Successfully"));
      await refetch();
    } else {
  ...
  const handleSavingRow = async (e: any) => {
    ...
    if (e.changes && e.changes.length > 0) {
      ...
      if (type === "remove") {
        ...
        const data: DirCA = e.changes[0].data!;
  ...
  const handleUploadFile = async (file: File, progressCallback?: Function) => {
    const resp = await api.uploadDirCA(file);
    if (resp.isSuccess) {
  ...
  const handleDownloadTemplate = async () => {
    const resp = await api.downloadDirCA();
    if (resp.isSuccess) {
      ...
      <AdminContentLayout.Slot name={"Header"}>
        <PageHeaderLayout>
          <PageHeaderLayout.Slot name={"Before"}>
            <div className="font-bold dx-font-m">{t("Dir CA Management")}</div>
