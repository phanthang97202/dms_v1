...
export const useAuthService = () => {
  ...
  const signOut = useCallback(() => {
    ...
    window.open(`${ssoDomain}/account/signout/${client_id}`, '_self', 'noreferrer')
    window.close();
  }, [logout]);
  ...
  const loginIgoss = useCallback(async (networkId: string) => {
    ...
    if(resp.Success) {
      // get client gate
      const clientGate = await entryGateApi.getNetworkInfo(networkId, resp.Data.OrgId, resp.Data.CurrentUser)
