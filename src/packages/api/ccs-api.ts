 ...
 */
localeApiBase.interceptors.response.use(function (response) {
...
export const localeApi = {
  loadLocaleData: async () => {
    return await ccsApiBase.post<Response<LocaleData>>(`${ccsDomain}/api/localization/GetData`, {
  ...
  addLocaleData: async (values: { cate: string, value: string; }[]) => {
    return await ccsApiBase.post<Response<LocaleData>>(`${ccsDomain}/api/localization/AddData`, {
      solutionCode: import.meta.env.VITE_SOLUTION_CODE,
      values: values,
    }, {
      headers: {
...
export const ccsApi = {
  getSessionInfo: async (networkId: string) => {
    ...
    const response = await ccsApiBase.get<Response<ISession>>("/api/Account/GetSessionInfo", {
      params: {
        networkId,
        token,
      }
  ...
  getNetworks: async () => {
    const token = localStorage.getItem("token");
    const response = await ccsApiBase.get<Response<IOrg[]>>("/api/Account/GetMyNetworks", {
      params: {
        token
      }
    });
  ...
  getPermissions: async () => {
    const token = localStorage.getItem("token");
    const response = await ccsApiBase.post<Response<any>>("/api/Permissions", {
      params: {
        token
      }
    });
